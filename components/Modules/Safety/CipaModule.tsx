...
export const CipaModule: React.FC<CipaModuleProps> = ({ collaborators, activeBranch, activeCompany }) => {
  ...
  const [terms, setTerms] = useState<CipaTerm[]>([
    { id: 't1', year: '2023/2024', startDate: '2023-10-07', endDate: '2024-10-07', branchId: 'b1', status: 'FINISHED' },
    { id: 't2', year: '2024/2025', startDate: '2024-10-07', endDate: '2025-10-07', branchId: 'b1', status: 'ACTIVE' }
  ]);
  ...
  useEffect(() => {
    if (selectedTermId) {
      ...
      // Simulação de dados para o termo selecionado
      if (meetings.length === 0) {
        setMeetings([
          { id: 'm1', termId: selectedTermId, date: '2024-11-10', title: 'Reunião de Instalação e Posse', description: 'Definição do cronograma anual e posse dos membros.', type: 'ORDINARY' }
        ]);
      }
    }
  }, [selectedTermId]);
  ...
  const saveMember = (e: React.FormEvent) => {
    ...
    const newMember: Cipeiro = {
      id: editingItem?.id || Math.random().toString(36).substr(2, 9),
      termId: selectedTermId!,
      collaboratorId: data.get('collaboratorId') as string,
      cipaRole: data.get('cipaRole') as CipaRole,
      origin: data.get('origin') as CipaOrigin,
      votes: Number(data.get('votes')) || 0
    };
    if (editingItem) {
      setCipeiros(cipeiros.map(c => c.id === editingItem.id ? newMember : c));
    } else {
      setCipeiros([...cipeiros, newMember]);
    }
  ...
  const savePlan = (e: React.FormEvent) => {
    e.preventDefault();
    const data = new FormData(e.currentTarget);
    const newPlan: CipaActionPlan = {
      id: editingItem?.id || Math.random().toString(36).substr(2, 9),
      meetingId: data.get('meetingId') as string,
      description: data.get('description') as string,
      deadline: data.get('deadline') as string,
      responsibleId: data.get('responsibleId') as string,
      status: data.get('status') as any
    };
    if (editingItem) {
      setActionPlans(actionPlans.map(p => p.id === editingItem.id ? newPlan : p));
    } else {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => {
                ...
                setTerms([...terms, { id: newId, year: 'Novo Pleito', startDate: '', endDate: '', branchId: activeBranch.id, status: 'ELECTION' }]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative z-10 p-8 animate-in zoom-in border border-emerald-100">
            ...
            <form onSubmit={savePlan} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4"><input type="date" required name="deadline" className="bg-emerald-50 p-4 rounded-xl font-black" /><select name="responsibleId" className="bg-emerald-50 p-4 rounded-xl font-black">{collaborators.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        ...
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 print:hidden">
          ...
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-10 text-center space-y-4 relative z-10 border border-red-50 shadow-2xl animate-in zoom-in">
              ...
              <div className="flex gap-2 pt-4">
                  ...
                  <button onClick={() => { setTerms(terms.filter(t => t.id !== termToDelete.id)); setTermToDelete(null); if (selectedTermId === termToDelete.id) setSelectedTermId(null); }} className="flex-1 bg-red-600 text-white py-3 rounded-xl font-black text-[10px] uppercase shadow-lg tracking-widest">Sim, Excluir</button>
              </div>
          </div>
        </div>
      )}
